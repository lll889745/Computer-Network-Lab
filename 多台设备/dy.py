...
# 绑定本机 IP 和端口
listen_ip = devices['dy']['ip']
listen_port = devices['dy']['port']
socket.bind((listen_ip, listen_port))

# 要发送的数据
message = 'Hello mxz! I am dy.'
...
def send_data():
    ...
    while True:
        ...
        # 发送数据
        socket.sendto(json.dumps(packet).encode(), (devices['mxz']['ip'], devices['mxz']['port']))
        sent_packets += 1
...
def receive_data():
    ...
    while True:
        ...
        # 如果接收到的数据包来自 zsy，则回复
        elif received_packet['from'] == 'zsy':
            reply_packet = {
                'id': sent_packets,
                'time_received': time.time(),
                'time_sent': received_packet['time_sent'],
                'message': 'dy receive from zsy',
                'from': device_name,
            }
            socket.sendto(json.dumps(reply_packet).encode(), (devices['zsy']['ip'], devices['zsy']['port']))
